...
/* ─── Card icon ────────────────────────────────────────────────────────────────
   Emoji were used here originally, but the site sets `font-family: 'DM Sans'`
        ...
        <RevealGroup className="rgrid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 28 }}>
          {steps.map((s) => (
            <RevealItem key={s.n} style={{ borderTop: `1px solid ${C.hairInk}`, paddingTop: 18 }}>
              <div style={{ fontSize: 40, fontWeight: 500, fontFamily: "'DM Sans',monospace", color: C.orange, letterSpacing: "-2px", marginBottom: 14, lineHeight: 1 }}>
                {s.n}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10, fontSize: 11, fontFamily: "'DM Sans',monospace", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em", color: "#6E6E64" }}>
                • Step {s.n}
              </div>
  ...
  return (
    <section style={{ padding: '96px 40px', background: C.paper }}>
      <Reveal className="rsplit" style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        >
          {/* SVG, not an emoji: 'DM Sans' carries no emoji glyph, so these
          ...
          <div style={{
            ...
            fontSize: 11, fontFamily: "'DM Sans', monospace", fontWeight: 500,
...
export function CTA() {
  return (
    <section style={{ padding: "56px 24px 120px", background: C.paper }}>
      <Reveal style={{ maxWidth: 1160, margin: "0 auto", background: "linear-gradient(120deg, #FF5A1F 0%, #F43F8E 55%, #8B5CF6 100%)", borderRadius: 40, padding: "96px 48px", textAlign: "center", boxShadow: "0 30px 80px rgba(139,92,246,0.28)" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 24, fontSize: 12, fontFamily: SANS, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.14em", color: "rgba(255,255,255,0.85)" }}>
          <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#fff", display: "inline-block" }} />
          Get started
        </div>
        <h2 style={{ fontSize: "clamp(38px,5.4vw,76px)", fontWeight: 700, color: "#fff", letterSpacing: "-0.04em", lineHeight: 0.98, marginBottom: 24 }}>
          Ready to grow your Etsy shop?
        </h2>
        <p style={{ fontSize: 19, color: "rgba(255,255,255,0.85)", marginBottom: 40, maxWidth: 480, marginLeft: "auto", marginRight: "auto", lineHeight: 1.5 }}>
        ...
        <div style={{ display: "flex", gap: 24, justifyContent: "center", alignItems: "center", flexWrap: "wrap" }}>
          <Link href="/register" style={{ background: "#fff", color: C.orange, textDecoration: "none", padding: "16px 34px", borderRadius: 12, fontSize: 16, fontWeight: 700, letterSpacing: "-0.01em", boxShadow: "0 8px 24px rgba(17,24,39,0.18)", transition: "opacity 0.18s" }}
            onMouseEnter={(e) => (e.currentTarget.style.opacity = "0.9")}
            onMouseLeave={(e) => (e.currentTarget.style.opacity = "1")}>
            Start free — no card
          </Link>
          <a href="#keywords" style={{ color: "#fff", fontSize: 16, fontWeight: 600, textDecoration: "underline", textUnderlineOffset: 5, textDecorationColor: "rgba(255,255,255,0.5)" }}>
    ...
    <footer style={{ background: C.ink, padding: "88px 40px 40px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div className="rgrid-4" style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 1fr", gap: 48, paddingBottom: 48, borderBottom: `1px solid rgba(245,245,235,0.14)` }}>
          <div>
            <div style={{ fontSize: 26, fontWeight: 700, color: "#ffffff", letterSpacing: "-0.04em", marginBottom: 20 }}>Ranktsy<span style={{ color: "#FF5A00" }}>.</span></div>
