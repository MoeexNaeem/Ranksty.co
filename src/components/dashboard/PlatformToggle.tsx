    ...
    <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
      {PLAT.map(({key,label,color}) => (
        <button key={key} onClick={()=>toggle(key)}
          style={{ display:'flex', alignItems:'center', gap:6, fontSize:13, fontWeight:500, fontFamily:"'DM Sans',sans-serif", background:'none', border:'none', cursor:'pointer', padding:0, opacity:active.includes(key)?1:0.35, transition:'opacity 0.15s' }}>
