              ...
              <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
                {listing.tags.map(t => (
                  <button key={t} onClick={() => navigator.clipboard?.writeText(t)} title="Click to copy"
                    style={{ fontSize: 14, fontFamily: MONO, color: C.orange, background: C.orangeFaint, border: `1px solid rgba(255,90,31,0.22)`, padding: '7px 14px', borderRadius: 100, cursor: 'pointer' }}>{t}</button>
                ))}
              </div>
            </Field>
