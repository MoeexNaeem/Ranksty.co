    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.canvas }}>
      ...
      <aside className={`rdash-aside${navOpen ? ' rdash-open' : ''}`} style={{ width: 244, background: C.paper, borderRight: `1px solid ${C.ash}`, display: 'flex', flexDirection: 'column', flexShrink: 0, position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '20px 22px 16px', borderBottom: `1px solid ${C.ash}` }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none' }}>
            <span style={{ fontSize: 23, fontWeight: 700, color: C.ink, letterSpacing: '-0.04em' }}>Ranktsy<span style={{ color: '#FF5A00' }}>.</span></span>
          </Link>
        </div>
        ...
        <nav style={{ flex: 1, padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {GROUPS.map(group => (
            <div key={group} style={{ marginBottom: 12 }}>
              <p className="rlabel" style={{ fontSize: 10, fontFamily: "'DM Sans',sans-serif", fontWeight: 600, color: C.stone, textTransform: 'uppercase', letterSpacing: '0.11em', padding: '4px 14px', marginBottom: 4 }}>{group}</p>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div className="rdash-topbar" style={{ padding: '15px 28px', borderBottom: `1px solid ${C.hair}`, background: C.paper, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, position: 'sticky', top: 0, zIndex: 5 }}>
          <div style={{ display: 'flex', alignItems: 'center', minWidth: 0, gap: 14 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <h1 style={{ fontSize: 22, fontWeight: 700, color: C.ink, letterSpacing: '-0.03em', marginBottom: 2 }}>{activeInfo.label}</h1>
              <p className="rdash-desc" style={{ fontSize: 13.5, color: C.graphite }}>
                {activeTab === 'overview' ? `Welcome back, ${user?.name?.split(' ')[0] ?? 'there'} 👋` : activeInfo.description}
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
            <span className="rdash-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 12, background: '#ECFDF5', color: '#059669', padding: '7px 13px', borderRadius: 999, fontWeight: 600 }}>
              <span style={{ position: 'relative', width: 7, height: 7, borderRadius: '50%', background: '#10B981', display: 'inline-block' }}>
                <span className="shimmer" style={{ position: 'absolute', inset: -2, borderRadius: '50%', border: '1.5px solid #10B981', opacity: 0.5 }} />
              </span>
              Etsy API · Live
            </span>
            {/* Notification bell */}
            <button className="rhide-sm" aria-label="Notifications" style={{ position: 'relative', width: 40, height: 40, borderRadius: 11, border: `1px solid ${C.ash}`, background: C.paper, color: C.graphite, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
              ...
              <span style={{ position: 'absolute', top: 9, right: 10, width: 7, height: 7, borderRadius: '50%', background: C.orange, border: '2px solid #fff' }} />
            </button>
            {/* User chip */}
            {user && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '5px 6px 5px 11px', borderRadius: 12, border: `1px solid ${C.ash}`, background: C.paper }}>
                <div className="rhide-sm" style={{ textAlign: 'right', lineHeight: 1.15 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: C.ink, whiteSpace: 'nowrap' }}>{user.name ?? 'User'}</p>
                  <p style={{ fontSize: 11, color: C.graphite, textTransform: 'capitalize' }}>{user.plan ?? 'free'} Plan</p>
                </div>
                <span style={{ width: 34, height: 34, borderRadius: '50%', background: activeHue, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13.5, fontWeight: 700, flexShrink: 0 }}>
        ...
        <div className="rdash-footer" style={{ padding: '12px 28px', borderTop: `1px solid ${C.hair}`, background: C.paper, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <p style={{ fontSize: 11.5, color: '#8a8a82', fontFamily: "'DM Sans',monospace", lineHeight: 1.5, margin: 0 }}>
            The term &apos;Etsy&apos; is a trademark of Etsy, Inc. This application uses the Etsy API but is not endorsed or certified by Etsy, Inc.
          </p>
          <Link href="/methodology" style={{ fontSize: 11.5, color: C.orange, fontFamily: "'DM Sans',monospace", textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}>How our data works →</Link>
        </div>
      </main>
    </div>
  )
