        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 14px', borderBottom:'1px solid rgba(0,0,0,0.06)', flexShrink:0 }}>
            ...
            <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
              <span style={{ fontSize:10.5, background:C.orange, color:C.snow, padding:'3px 10px', borderRadius:999, fontFamily:"'DM Sans',monospace" }}>Etsy US</span>
              <span style={{ fontSize:10, color:'#bbb', fontFamily:"'DM Sans',monospace" }}>Live data</span>
            </div>
          </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:9 }}>
                  {isLoading||!stats ? [0,1,2].map(i=><Skel key={i} h={74} />) : stats.map(s=>(
                    <div key={s.label} style={{ background:C.warmGray, borderRadius:10, padding:'11px 13px' }}>
                      <p style={{ fontSize:9.5, fontFamily:"'DM Sans',monospace", color:'#999', textTransform:'uppercase' as const, letterSpacing:'0.05em', marginBottom:4 }}>{s.label}</p>
