  ...
  const options = useMemo<ChartOptions<'line'>>(() => ({
    ...
    plugins: {
      ...
      tooltip: {
        ...
        bodyFont: { size: 12.5, family: "'DM Sans',sans-serif" },
        cornerRadius: 9, boxPadding: 5, usePointStyle: true,
        callbacks: { label: c => `  ${c.dataset.label}: ${Number(c.parsed.y).toLocaleString()}` },
      },
    },
    scales: {
      x: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 11, family: "'DM Sans',sans-serif" }, color: '#93938A', maxRotation: 0, autoSkipPadding: 12 } },
      y: {
        grid: { color: 'rgba(61,62,59,0.07)' }, border: { display: false }, beginAtZero: true,
        ticks: { font: { size: 11, family: "'DM Sans',sans-serif" }, color: '#93938A', maxTicksLimit: 6, padding: 8, callback: v => Number(v).toLocaleString() },
      },
    },
  }), [])
